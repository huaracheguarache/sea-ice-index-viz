...
def visualisation():
    ...
    reference_period_selector = pn.widgets.Select(
        name='Reference period of percentiles and median:',
    ...
    zoom_shortcuts = pn.widgets.MenuButton(
        name='Zoom shortcuts:',
        items=[
            ('Year', 'year'),
    ...
    plot.yaxis.ticker = AdaptiveTicker(
        base=10, mantissas=[1, 2], num_minor_ticks=4, desired_num_ticks=10
    )
    ...
    plot.hspan(
        y=0, line_color='#d0d0d0', line_dash=value([10, 10]), line_width=3
    )
    ...
    yearly_min = plot.scatter(
        x='doy',
        y='value',
        color='colour',
        size=6,
        source=data.cds_yearly_min,
        visible=False,
    )
    yearly_max = plot.scatter(
        x='doy',
        y='value',
        color='colour',
        size=6,
        source=data.cds_yearly_max,
        visible=False,
    )
    ...
    all_yearly_glyphs = [glyph[0] for year, glyph in yearly] + [
        last_year_outline
    ]
    tooltips = Tooltips(
    ...
    number_of_lines = 6
    label_height = (
        float(info_label.text_font_size.rstrip('px'))
        * info_label.text_line_height
        * number_of_lines
    )
